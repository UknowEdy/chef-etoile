...
export default function App() {
  ...
  const SuccessView = () => (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-white text-center">
        ...
        <p className="text-stone-500 mb-8 max-w-xs mx-auto">
            Vous avez été redirigé vers WhatsApp. Veuillez envoyer le message pré-rempli pour finaliser le paiement avec notre agent.
        </p>
        <button 
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {currentView !== 'ADMIN' && currentView !== 'SUCCESS' && (
        <Header onNavigate={navigateTo} />
      )}

      <main className="flex-grow">
        {currentView === 'HOME' && (
          <Landing onSelectPlan={handlePlanSelection} />
        )}
      ...
      {currentView === 'HOME' && (
        <footer className="bg-chef-black text-stone-400 py-10 text-center text-sm border-t border-white/10">
            <p className="font-serif text-white text-lg mb-2">Chef<span className="text-chef-gold">★</span></p>
            <p className="mb-6">Lomé, Togo. &copy; 2023 Tous droits réservés.</p>
